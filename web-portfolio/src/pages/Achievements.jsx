      ...
      <div className="relative mx-auto max-w-[1300px] px-6 py-6 grid grid-cols-1 lg:grid-cols-[280px_minmax(0,1fr)_300px] gap-6" style={{ transform: `scale(${visualCfg.hudScale})` }}>
        ...
        <main className="relative rounded-md border border-white/15 bg-black/20 backdrop-blur-sm overflow-hidden">
          ...
          <div className="p-5 grid grid-cols-1 lg:grid-cols-[280px_minmax(0,1fr)] gap-6">
            ...
            {/* Right col: lists */}
            <div className="space-y-4">
              <Section title="Achieved" items={computed.earned} />
              <Section title="In Progress" items={computed.inProgress} />
              <Section title="To Achieve" items={computed.todo} subtle />
            </div>
          </div>
        </main>
...
function Section({ title, items, subtle }) {
  return (
    <div className="rounded-md border border-white/10 bg-black/20">
      ...
      <div className="p-3 space-y-3">
        ...
        {items.map(it => (
          <div key={it.id} className={`relative overflow-hidden rounded border p-3 transition-colors ${it.rarity==='legendary'?'border-yellow-500/40 bg-yellow-500/10':it.rarity==='epic'?'border-fuchsia-500/40 bg-fuchsia-500/10':it.rarity==='rare'?'border-sky-500/40 bg-sky-500/10':'border-white/10 bg-black/30'} ${subtle?'opacity-80':''}`}>
